...
def manual_centerline_widget(state: ViewerState, fig: Any, im: Any) -> dict:
    ...
    def extract_centerline(event):
        ...
        centerline_binary = extract_centerline(masked_elevation, state.vmin, state.vmax, smooth_sigma=3.0)
        
        # Convert to LineString in DISPLAY pixel coordinates (no transform)
        centerline_display = skeleton_to_linestring(centerline_binary, transform=None)
        ...
        centerline_geom = skeleton_to_linestring(centerline_binary, transform=scaled_transform)
...
def centerline_widget(state: ViewerState, fig: Any, im: Any) -> dict:
    ...
    centerline_geom = skeleton_to_linestring(centerline, state.transform)
...
def derive_centerline(
    ...
    elements = query_osm_waterways(bounds_wgs84)
    ...
    linestrings = build_linestrings_from_osm(elements)
    ...
    if snap_to_channel:
        ...
        centerline_raster_crs = snap_centerline_to_channel(
...
def interactive_osm_centerline(
    ...
    if hillshade_params is not None:
        alpha, exaggeration, altitude = hillshade_params
        hillshade = compute_hillshade(data, altitude=altitude, z_factor=exaggeration)
        ax.imshow(
            hillshade,
            cmap='gray',
            alpha=alpha,
    ...
    def recompute(event):
        ...
        try:
            snapped = snap_centerline_to_channel(
